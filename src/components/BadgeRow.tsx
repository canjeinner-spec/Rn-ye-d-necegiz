...
export function BadgeRow({ badges, size = 26 }: { badges: BadgeItem[]; size?: number }) {
  ...
  return (
    <View style={{ flexDirection: "row", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
      {badges.map((b) => (
        <Pressable key={b.type + (b.lvl ?? "")} onPress={() => setInfo(b)}>
          {b.type === "agency" ? (
      ...
      <Modal visible={!!info} transparent animationType="fade" onRequestClose={() => setInfo(null)}>
        <Pressable
          ...
          style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 30, backgroundColor: "rgba(3,3,8,.5)" }}
        >
          {cur && info && (
            <GlassPanel style={{ width: "100%", maxWidth: 244, padding: 18, paddingTop: 20, alignItems: "center" }} radius={22}>
              <View style={{ marginBottom: 11 }}>
                {info.type === "agency" ? (
                  ...
                  <Badge type={cur.icon as BadgeType} size={50} lvl={info.type === "level" ? 12 : undefined} />
                )}
              </View>
