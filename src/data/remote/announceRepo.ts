...
export type Banner = { id: number; baslik: string; aciklama?: string; foto?: string; sira: number };
...
export async function listBanners(): Promise<Banner[]> {
  ...
  const { data, error } = await sb
    ...
    .select("id, baslik, aciklama, foto_url, sira")
    .eq("aktif", true)
    .order("sira", { ascending: true })
    .order("id", { ascending: false });
  if (error) throw error;
  return ((data as { id: number; baslik: string; aciklama: string | null; foto_url: string | null; sira: number }[]) ?? []).map((r) => ({
    id: r.id, baslik: r.baslik, aciklama: r.aciklama || undefined, foto: r.foto_url || undefined, sira: r.sira,
  }));
}

export async function createBanner(baslik: string, aciklama?: string, foto?: string, sira = 0): Promise<number> {
  const sb = requireSupabase();
  const { data, error } = await sb.rpc("banner_ekle", { p_baslik: baslik.trim(), p_aciklama: aciklama ?? null, p_foto: foto ?? null, p_sira: sira });
  if (error) throw error;
  return data as number;
}
export async function updateBanner(id: number, baslik: string, aciklama: string | null, foto: string | null, sira: number): Promise<void> {
  const sb = requireSupabase();
  const { error } = await sb.rpc("banner_guncelle", { p_id: id, p_baslik: baslik, p_aciklama: aciklama, p_foto: foto, p_sira: sira });
  if (error) throw error;
}
